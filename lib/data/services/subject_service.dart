...
class SubjectService {
  ...
  Future<AppResult<BulkImportResult>> bulkAddSubjects({
    required List<String> names,
    required String institutionId,
  }) async {
    if (names.isEmpty) return AppResult.success(const BulkImportResult(imported: 0, skippedReasons: []));
    try {
      final existing = await _client.from('subjects').select('name').eq('institution_id', institutionId);
      ...
      if (toInsert.isNotEmpty) {
        await _client
            .from('subjects')
            .insert(toInsert.map((n) => {'name': n, 'institution_id': institutionId}).toList());
      }
      ...
      return AppResult.failure('Не удалось выполнить импорт предметов.');
    }
  }
